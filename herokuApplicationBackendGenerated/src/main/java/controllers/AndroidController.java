...
private GenericDAOMongoDB<Android> genericDAOMongoDBAndroid;
	...
	public void list() {
		try {
			result.use(ExtJSJson.class).from(genericDAOMongoDBAndroid.getEntities())
					.success().serialize();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	// POST /android/saveOrUpdate => this method receives a POST request
		...
		public void create(Android android) {
			...
			if(android.getId()!=null){
					androidDTO = genericDAOMongoDBAndroid.getEntityById(android.getId());
			}
			...
			try {
				...
				if (errors.isEmpty()) {
					if (androidDTO.getId() == null) {
						...
						genericDAOMongoDBAndroid.updateEntity(androidDTO, androidDTO.getId());
					}
				} else {
	...
	@Get
			...
			public void retrieve(String id) {
				AjaxReturn ajaxReturn = new AjaxReturn();
				try{
					result.use(ExtJSJson.class).from(genericDAOMongoDBAndroid.getEntityById(id))
						.serialize();
				}catch(Exception ex){
