	...
	// POST /connectivity/saveOrUpdate => this method receives a POST request
		...
		public void create(Connectivity connectivity) {
			...
			try {
				...
				if (errors.isEmpty()) {
					if (connectivityDTO.getId() == null) {
						connectivityDTO.setId(ObjectId.get().toString());
						genericDAOMongoDBConnectivity.insertEntity(connectivityDTO);
					} else {
						genericDAOMongoDBConnectivity.updateEntity(connectivityDTO, connectivityDTO.getId());
					}
				} else {
